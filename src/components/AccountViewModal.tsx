...
import { X, Mail, Phone, User, Building, Briefcase, DollarSign, Shield, UserCheck } from "lucide-react";

interface Props {
  employee: AccountResponse;
  onClose: () => void;
}

const AccountViewModal = ({ employee, onClose }: Props) => {
          ...
          <div className="bg-blue-50 rounded-xl p-6 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex items-center gap-3 p-3 bg-white rounded-lg">
                ...
                <div>
                  <p className="text-sm text-gray-600">ชื่อผู้ใช้งาน</p>
                  <p className="font-medium">{employee.username || "ยังไม่สร้าง"}</p>
                </div>
              </div>
              <div className="flex items-center gap-3 p-3 bg-white rounded-lg">
                <Shield size={20} className="text-purple-500" />
                <div>
                  <p className="text-sm text-gray-600">สิทธิ์ผู้ใช้</p>
                  <div className={`inline-block px-2 py-1 rounded-full text-xs font-medium border ${getRoleColor(employee.role)}`}>
                    {getRoleText(employee.role) || "ยังไม่ได้กำหนด"}
                  </div>
                </div>
              </div>
            </div>
